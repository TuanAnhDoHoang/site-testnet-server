...
class SuinsClient {
  ...
  // }
  async getPriceList() {
    if (!this.config.suins)
      throw new Error("Suins object ID is not set");
    if (!this.config.packageId)
      throw new Error("Price list config not found");
    const priceList = await this.client.getDynamicFieldObject({
      parentId: this.config.suins,
      name: {
        type: (0, import_constants.getConfigType)(
          this.config.packageIdV1,
          (0, import_constants.getPricelistConfigType)(this.config.packageIdPricing)
        ),
  ...
  async getRenewalPriceList() {
    ...
    const priceList = await this.client.getDynamicFieldObject({
      ...
      name: {
        type: (0, import_constants.getConfigType)(
          this.config.packageIdV1,
          (0, import_constants.getRenewalPricelistConfigType)(this.config.packageIdPricing)
        ),
        value: { dummy_field: false }
      }
    });
  ...
  async getCoinTypeDiscount() {
    ...
    const dfValue = await this.client.getDynamicFieldObject({
      ...
      name: {
        type: (0, import_constants.getConfigType)(
          this.config.packageIdV1,
          (0, import_constants.getCoinDiscountConfigType)(this.config.payments.packageId)
        ),
        value: { dummy_field: false }
      }
    });
  ...
  async getPriceInfoObject(tx, feed) {
    ...
    const connection = new import_pyth_sui_js.SuiPriceServiceConnection(endpoint);
    const priceIDs = [
      feed
      // ASSET/USD price ID
    ];
    ...
    const client = new import_pyth_sui_js.SuiPythClient(this.client, pythStateId, wormholeStateId);
