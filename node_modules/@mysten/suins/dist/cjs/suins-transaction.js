...
class SuinsTransaction {
  constructor(client, transaction) {
    ...
    this.calculatePrice = (baseAmount, paymentType, priceInfoObjectId) => {
      const config = this.suinsClient.config;
      return this.transaction.moveCall({
    ...
    this.handleBasePayment = (paymentIntent, payment, paymentType) => {
      const config = this.suinsClient.config;
      return this.transaction.moveCall({
    ...
    this.handlePayment = (paymentIntent, payment, paymentType, priceInfoObjectId, maxAmount = import_constants.MAX_U64) => {
      const config = this.suinsClient.config;
      return this.transaction.moveCall({
        ...
        arguments: [
          ...
          // This is the maximum user is willing to pay
        ],
        typeArguments: [paymentType]
      });
    };
    this.finalizeRegister = (receipt) => {
      const config = this.suinsClient.config;
      return this.transaction.moveCall({
    ...
    this.finalizeRenew = (receipt, nft) => {
      const config = this.suinsClient.config;
      return this.transaction.moveCall({
    ...
    this.calculatePriceAfterDiscount = (paymentIntent, paymentType) => {
      const config = this.suinsClient.config;
      return this.transaction.moveCall({
    ...
    this.generateReceipt = (params) => {
      ...
      if (baseAssetPurchase) {
        ...
        if (!params.coin)
          throw new Error("coin input is required");
     ...
     */
    this.applyCoupon = (intent, couponCode) => {
      const config = this.suinsClient.config;
      return this.transaction.moveCall({
     ...
     */
    this.applyDiscount = (intent, discountInfo) => {
      const config = this.suinsClient.config;
  ...
  createSubName({
    ...
    if (!this.suinsClient.config.suins)
      throw new Error("SuiNS Object ID not found");
  ...
  removeLeafSubName({ parentNft, name }) {
    ...
    if (!(0, import_helpers.isSubName)(name))
      throw new Error("This can only be invoked for subnames");
    if (!this.suinsClient.config.suins)
      throw new Error("SuiNS Object ID not found");
    if (!this.suinsClient.config.subNamesPackageId)
      throw new Error("Subnames package ID not found");
  ...
  setUserData({
    ...
    if (isSubname && !this.suinsClient.config.tempSubdomainsProxyPackageId)
      throw new Error("Subnames proxy package ID not found");
    if (!Object.values(import_constants.ALLOWED_METADATA).some((x) => x === key))
